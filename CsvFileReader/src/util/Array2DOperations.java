...
/**
 * class give tools to compare and deduplicate 2Darrays
...
public class Array2DOperations {
	 ...
	 * @param firstFileColumn - first file first column index
	 * @param secondFileColumn - second file first column index
	...
	public String[][] searchForDoubleData( CsvFile firstFile,
		...
		while ( petle2 <= secondFileTable.length ) {
			...
			if ( ( firstFileTable[ petle1 ][ firstFileColumn ].equals
					( secondFileTable[ petle2 ][ secondFileColumn ] ) ) ) {
				break;
			}
			petle2++;
		}
	 ...
	 * @param firstFileColumn - first file first column index
	 * @param secondFileColumn - second file first column index
	 * @param firstFileSecondColumn - first file second column index
	 * @param secondFileSecondColumn - second file second column index
	 * @return unique value of date in first file
	 */
	public String[][] searchForDoubleData( CsvFile firstFile,
			CsvFile secondFile, int firstFileColumn, int secondFileColumn, 
			int firstFileSecondColumn, int secondFileSecondColumn )	{
		doneCompare = false;
		// compare files
		String[][] firstFileTable = firstFile.getCsvFileTableModel();
		String[][] secondFileTable = secondFile.getCsvFileTableModel();
		//create new files with unique values
		final String[][] uniqueTmp = new String[ firstFileTable.length ][ firstFileTable[ 0 ].length ];
		final String[][] doubleData = new String[ firstFileTable.length ][ firstFileTable[ 0 ].length ];
		int petle1 = 0;
		int petle2 = 0;
		int copy = 0;
		int newDate = 0;
		...
		while ( petle1 < firstFileTable.length - 1 ) {
			...
			while ( petle2 <= secondFileTable.length ) {
				...
				if (( firstFileTable[ petle1 ][ firstFileColumn ]
						.equals( secondFileTable[ petle2 ][ secondFileColumn ]) && firstFileTable[ petle1 ][ firstFileSecondColumn ]
						.equals( secondFileTable[ petle2 ][ secondFileSecondColumn ]))) {
					for (int i = 0; i < firstFileTable[ petle1 ].length; i++ ) {
						doubleData[ copy ][ i ] = firstFileTable[ petle1 ][ i ];
					}
					copy++;
					break;
				}
		...
		while ( petle2 <= secondFileTable.length ) {
			...
			if ( ( firstFileTable[ petle1 ][ firstFileColumn ]
					...
					.equals( secondFileTable[ petle2 ][ secondFileSecondColumn ]))) {
				for (int i = 0; i < firstFileTable[ firstFileTable.length - 1 ].length; i++ ) {
					doubleData[ copy ][ i ] = firstFileTable[ firstFileTable.length - 1 ][ i ];
		...
		progress3.close();
		removeNullFron2DArray( uniqueTmp );
		doneCompare = true;
		return uniqueTmp;
	}
